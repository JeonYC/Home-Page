 ...
 * HanJs
 * Author : teensoft
 * HanJs is Under the GPL 3.0 license
 */
...
/**
 * Han.visible
 * 특정 element를 보고 있을 때 어떠한 동작을 수행하도록 합니다.
 * example
 * var HanVisible = new Han.visible({
 * 	elementClass : '.Han_header_element',
 * 	onChangeView : function(isWatching) {
 * 		if(isWatching) {
 * 			// 특정 element를 보고 있을때의 소스
 * 		} else {
 * 			// 특정 element를 보고 있지 않을때의 소스
 * 		}
 * 	}
 * });
 * HanVisible.init();
...
Han.visible = function(options){
	this.Obj = options
};
Han.header.prototype = {
	init : function() {
		var visibleView = new eg.Visible(document , {
			targetClass : this.Obj.elementClass,
			expandSize : 0
		}).on("change" , function(e) {
			this.Obj.onChangeView($(e.visible).attr('class') == this.Obj.elementClass);
		}
	}
};